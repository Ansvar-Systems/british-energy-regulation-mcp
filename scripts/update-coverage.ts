 ...
 *
 * Reads the gb-energy.db, queries each table for row counts,
...
const DB_PATH = process.env["GB_ENERGY_DB_PATH"] ?? join(__dirname, "..", "data", "gb-energy.db");
...
const SOURCE_QUERIES: Record<string, string> = {
  ofgem: "SELECT COUNT(*) as c FROM regulations WHERE regulator_id = 'ofgem'",
  hse: "SELECT COUNT(*) as c FROM regulations WHERE regulator_id = 'hse'",
  neso: "SELECT COUNT(*) as c FROM grid_codes",
  desnz: "SELECT COUNT(*) as c FROM decisions",
};
